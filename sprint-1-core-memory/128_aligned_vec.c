...
int main(void)
{
	...
	neon_a = vld1q_f32(&vec_a.x);
	neon_b = vld1q_f32(&vec_b.x);
	neon_sum = vaddq_f32(neon_a, neon_b);
	vst1q_f32(&vec_result_neon.x, neon_sum);

	print_vec("vec_result_neon", &vec_result_neon);

	printf("\nPerforming scalar addition for comparison:\n");
	...
	if (vec_result_neon.x == vec_result_scalar.x &&
		...
		printf("\nResults DO NOT match! Check your logic or "
		       "compilation.\n");
		return EXIT_FAILURE;
	}
