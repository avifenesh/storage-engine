 ...
 */
static uint32_t scalar_hash_function(const void *data, size_t len)
{
 ...
 */
static uint32_t neon_hash_function_impl(const void *data, size_t len)
{
	const unsigned char *bytes = (const unsigned char *)data;
	...
	if (full_chunks > 0) {
		...
		for (i = 0; i < full_chunks; i++) {
			...
			uint8x16_t data_vec = vld1q_u8(bytes + i * NEON_HASH_CHUNK_SIZE);
		...
		uint32x2_t hash_pair = vadd_u32(vget_low_u32(hash_vec), vget_high_u32(hash_vec));  
 ...
 */
static void neon_hash_batch_impl(const void **keys, size_t *key_lens,
				 uint32_t *hashes, size_t count)
{
	...
	for (i = 0; i + 3 < count; i += 4) {
		...
		size_t chunks = min_len / 4;  /* Process 4 bytes per key simultaneously */
		for (j = 0; j < chunks; j++) {
			/* Load 4 bytes from each of the 4 keys */
			uint32_t data[4];
			data[0] = *((uint32_t *)((unsigned char *)keys[i] + j * 4));
			data[1] = *((uint32_t *)((unsigned char *)keys[i + 1] + j * 4));
			data[2] = *((uint32_t *)((unsigned char *)keys[i + 2] + j * 4));
			data[3] = *((uint32_t *)((unsigned char *)keys[i + 3] + j * 4));
		...
		for (j = 0; j < 4; j++) {
			const unsigned char *bytes = (const unsigned char *)keys[i + j];
 ...
 */
void neon_hash_batch(const void **keys, size_t *key_lens, uint32_t *hashes, size_t count)
{
 ...
 */
int neon_key_compare(const void *key1, size_t key1_len, const void *key2, size_t key2_len)
{
	...
	if (key1_len >= 16 && may_use_simd()) {
		...
		return memcmp(bytes1 + full_chunks * 16, bytes2 + full_chunks * 16, remainder);
	}
#endif
