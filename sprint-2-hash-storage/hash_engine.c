...
int hash_engine_init(struct hash_engine *engine, int bucket_count) {
    engine->hash_buckets = malloc(bucket_count * sizeof(struct hash_bucket));
...
int hash_put(struct hash_engine *engine, const void *key, size_t key_len, const void *value, size_t value_len)
{
	(void)engine;
	(void)key;
	(void)key_len;
...
int hash_get(struct hash_engine *engine, const void *key, size_t key_len, const void **value, size_t *value_len)
{
	(void)engine;
	(void)key;
	(void)key_len;
...
int
hash_get_stats(struct hash_engine *engine, uint32_t *item_count, uint32_t *bucket_count,
