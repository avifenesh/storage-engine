...
test_basic_operations(void)
{
	...
	/* Test PUT operation */
	struct hash_operation put_op = { .key = key1,
					 .key_len = strlen(key1),
					 .value = value1,
	...
	/* Test GET operation */
	struct hash_operation get_op = { .key = key1,
					 .key_len = strlen(key1),
	...
	TEST_ASSERT(get_op.value_len == strlen(value1),
		    "Retrieved value length correct");
	TEST_ASSERT(memcmp(retrieved_value, value1, get_op.value_len) == 0,
		    "Retrieved value content correct");
	...
	TEST_ASSERT(memcmp(retrieved_value, new_value1, get_op.value_len) == 0,
		    "Updated value correct");

	/* Test item count unchanged after update */
	result = ioctl(fd, HASH_IOC_STATS, &stats);
	TEST_ASSERT(result == 0 && stats.item_count == 2,
		    "Item count unchanged after update");

	/* Test DELETE operation */
	struct hash_operation delete_op
	    = { .key = key1, .key_len = strlen(key1) };
	...
	result = ioctl(fd, HASH_IOC_STATS, &stats);
	TEST_ASSERT(result == 0 && stats.item_count == 1,
		    "Item count decremented after delete");
...
test_error_conditions(void)
{
	...
	char buffer[256];
	struct hash_operation get_op = { .key = nonexistent_key,
	...
	struct hash_operation delete_op
	    = { .key = nonexistent_key, .key_len = strlen(nonexistent_key) };
	...
	struct hash_operation put_op = { .key = NULL,
					 .key_len = 0,
					 .value = value,
...
test_performance(void)
{
	...
	for (i = 0; i < num_operations; i++) {
		...
		snprintf(value_buffer, sizeof(value_buffer),
			 "perf_value_%d_with_some_extra_data", i);
	...
	TEST_ASSERT(successful_ops == num_operations,
		    "All PUT operations successful");
