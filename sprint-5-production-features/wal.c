...
static int wal_update_size(wal_t *wal)
{
    long pos;
    if (!wal || !wal->fp) return -EINVAL;
    if (fflush(wal->fp) != 0) return -errno;
    pos = ftell(wal->fp);
...
int wal_append(wal_t *wal, const void *payload, size_t len, uint64_t *out_lsn)
{
    ...
    if (!wal || !wal->fp || (!payload && len)) return -EINVAL;
    ...
    if (fwrite(&hdr, sizeof(hdr), 1, wal->fp) != 1) return -errno;
    if (len && fwrite(payload, len, 1, wal->fp) != 1) return -errno;
...
int wal_flush(wal_t *wal)
{
    if (!wal || !wal->fp) return -EINVAL;
    if (fflush(wal->fp) != 0) return -errno;
    // fsync is platform specific; fflush is sufficient for this learning skeleton.
    return 0;
}
...
int wal_replay(const char *path, wal_apply_cb apply, void *ctx)
{
    ...
    while (1) {
        ...
        if (r != 1) { rc = 0; break; } // End or partial header; treat as end

        if (hdr.length > (64 * 1024 * 1024)) { rc = -EINVAL; break; }
        buf = NULL;
        if (hdr.length) {
            buf = malloc(hdr.length);
            if (!buf) { rc = -ENOMEM; break; }
            if (fread(buf, hdr.length, 1, fp) != 1) {
                // Torn/partial record — stop replay without error.
                free(buf); rc = 0; break;
            }
            csum = crc32c(buf, hdr.length);
            if (csum != hdr.checksum) { free(buf); rc = -EIO; break; }
        }
...
uint32_t crc32c(const void *data, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        for (int k = 0; k < 8; k++) {
            uint32_t mask = -(crc & 1);
            crc = (crc >> 1) ^ (0x82F63B78u & mask); // Castagnoli polynomial
        }
    }
    return ~crc;
}
