...
static int apply_cb(const wal_record_hdr_t *hdr, const void *data, void *ctx)
{
    static unsigned long long last;
    (void)data; (void)ctx;
    // Minimal check: LSN increases
    if (hdr->lsn <= last) return -1;
    last = hdr->lsn;
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    wal = wal_open(path, 0);
    if (!wal) { perror("wal_open"); return 1; }

    // Append records
    for (int i = 0; i < n; i++) {
        char buf[32]; int len = snprintf(buf, sizeof(buf), "rec_%d", i);
        if (wal_append(wal, buf, (size_t)len, NULL) != 0) { fprintf(stderr, "append failed\n"); }
    }
    wal_flush(wal);
    wal_close(wal);

    // Try a few truncation points
    cuts[0] = 0; cuts[1] = 1; cuts[2] = 17; cuts[3] = 128; cuts[4] = 1024;
    cuts_n = 5;
    for (size_t i = 0; i < cuts_n; i++) {
        // copy original file to temp
        snprintf(tmp, sizeof(tmp), "chaos_%zu.log", i);
        in = open(path, O_RDONLY); if (in < 0) continue;
        out = open(tmp, O_CREAT|O_TRUNC|O_WRONLY, 0644); if (out < 0) { close(in); continue; }
