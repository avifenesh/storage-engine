...
static pc_node_t **
find_slot(pcache_t *pc, uint64_t page_id)
{
	...
	for (size_t i = 0; i < pc->table_size; i++) {
		...
		if (!*slot || (*slot && (*slot)->page.page_id == page_id))
			return slot;
	}
	return NULL;
}
...
static void
lru_move_front(pcache_t *pc, pc_node_t *node)
{
	if (pc->lru_head == node)
		return;
	// detach
	...
	if (pc->lru_tail == node)
		pc->lru_tail = node->prev;
	// push front
	node->prev = NULL;
	node->next = pc->lru_head;
	if (pc->lru_head)
		pc->lru_head->prev = node;
	pc->lru_head = node;
	if (!pc->lru_tail)
		pc->lru_tail = node;
}
...
static void
lru_push_front(pcache_t *pc, pc_node_t *node)
{
	node->prev = NULL;
	node->next = pc->lru_head;
	if (pc->lru_head)
...
pcache_t *
pc_create(size_t capacity_pages)
{
	pcache_t *pc;
	size_t ts;
	if (capacity_pages == 0)
...
pc_page_t *
pc_get(pcache_t *pc, uint64_t page_id)
{
	pc_node_t **slot;
	pc_node_t *node;
	if (!pc)
...
void
pc_pin(pcache_t *pc, pc_page_t *page)
{
	(void)pc;
	if (page)
		page->pins++;
...
void
pc_unpin(pcache_t *pc, pc_page_t *page)
{
	(void)pc;
	if (page && page->pins > 0)
		page->pins--;
...
void
pc_mark_dirty(pcache_t *pc, pc_page_t *page)
{
	(void)pc;
	if (page)
		page->dirty = true;
...
int
pc_flush_dirty(pcache_t *pc, pc_flush_cb cb, void *ctx)
{
	if (!pc || !cb)
		return -EINVAL;
